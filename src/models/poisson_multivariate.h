class poisson_multivariate
{
    ...
    /* Here you must put the poisson_multivariate model parameters */
    cube mu;
    cube lambda;
    mat Sigma;
    ...
    poisson_multivariate(SBM & membership, poisson_multivariate::network & net)
    {
         ...
         * SBM case
         */
        n_parameters = 2 * membership.Z.n_cols * membership.Z.n_cols * net.adj.n_slices + net.adj.n_slices * net.adj.n_slices;
        lambda.set_size(membership.Z.n_cols, membership.Z.n_cols, net.adj.n_slices);
    ...
    poisson_multivariate(SBM_sym & membership, poisson_multivariate::network & net)
    {
        /* Here you must intialize the number of parameters (n_parameters) and
         * your model parameters, knowing the membership and the network in the
         * SBM_sym case
    ...
    Rcpp::List export_to_R()
    {
        ...
        Rcpp::List lambda_as_list(lambda.n_slices);
        for(unsigned int k=0;k<mu.n_slices;k++)
        {
            mu_as_list[k]=mu.slice(k);
            lambda_as_list[k] = lambda.slice(k);
        }
        values["mu"] = mu_as_list;
        values["lambda"] = lambda_as_list;
    ...
    poisson_multivariate(SBM &, const vec &);
    poisson_multivariate(SBM_sym &, const vec &);
    poisson_multivariate(LBM &, const vec &);
};
...
void e_fixed_step(SBM & membership,
                  poisson_multivariate & model,
                  poisson_multivariate::network & net,
                  mat & lZ)
{
    //cube X_tilde = apply_matrix_on_tubes(model.iL, net.adjZ);
    c//ube lambda_tilde = apply_matrix_on_tubes(model.iL, model.lambda);

    for(unsigned int k=0;k<net.adj.n_slices;k++)
    {
        lZ += net.adjZ.slice(k) * membership.Z * log(model.lambda.slice(k)).t()
       - net.MonesZ * membership.Z * model.lambda.slice(k).t()
       + net.adjZ.slice(k) * membership.Z * log(model.lambda.slice(k))
       - net.MonesZ * membership.Z * model.lambda.slice(k);
        /* This part is to be modified !!!!!!!!!!!!!!!!!!!!!
         * Modify to the Poisson version!!!!!!!!!
              ...
              poisson_multivariate::network & net)
{
    ...
    cube var_X(membership.Z.n_cols,membership.Z.n_cols,net.adj.n_slices);
    for(unsigned int k=0; k<net.adj.n_slices;k++)
    {
        model.lambda.slice(k) = (membership.Z.t() * net.adjZ.slice(k) * membership.Z)
                            /
                            provdiv;
        var_X(k) =   (membership.Z.t() * (net.adjZ.slice(k) % net.adjZ.slice(k)) * membership.Z)
                            /
                            provdiv- model.lambda.slice(k) % model.lambda.slice(k);
    }

    cube residual(net.adj.n_rows,net.adj.n_cols,net.adj.n_slices);
    for(unsigned int k=0;k<net.adj.n_slices;k++)
        residual.slice(k) = fill_diag(net.adj.slice(k) - membership.Z * model.lambda.slice(k) * membership.Z.t(),0);
    ...
    for(unsigned int k1=0;k1<net.adj.n_slices;k1++)
        for(unsigned int k2=0;k2<net.adj.n_slices;k2++)
            model.Sigma(k1,k2) = 1.0 / (net.adj.n_rows * (net.adj.n_cols - 1) ) *
                ...
                // This part should also be modified! Figure out how to calculate this!
    // THE ABOVE PART is used to calculate Sigma, BELOW should update mu by using Var(X)!
    // just to be sure:
...
/* If you have defined m_step(SBM &, poisson_multivariate &, poisson_multivariate::network &)
 * Then
 *     Useless
 * Else
 *     Usefull, Optional
...
// template<>
// inline
// double PL(poisson_multivariate & model,
//           SBM & membership,
//           poisson_multivariate::network & net)
...
/* If you have defined m_step(SBM_sym &, poisson_multivariate &, poisson_multivariate::network &)
 * Then
 *     Useless
 * Else
 *     Usefull, Optional
...
/* If you have defined m_step(LBM &, poisson_multivariate &, poisson_multivariate::network &)
 * Then
 *     Useless
 * Else
 *     Usefull, Optional
 ...
 *     - m_step(LBM &, poisson_multivariate &, poisson_multivariate::network)
 *     - m_step(SBM &, poisson_multivariate &, poisson_multivariate::network)
 *     - m_step(SBM_sym &, poisson_multivariate &, poisson_multivariate::network)
 *   is not defined
 * Then
 *     Usefull, Mandatory
 * Else
...
// inline
// vec poisson_multivariate::to_vector()
// {
// }
...
/* If m_step(SBM &, poisson_multivariate &, poisson_multivariate::network) is not defined
 * Then
 *     Usefull, Mandatory
 * Else
 *     Useless
...
// poisson_multivariate::poisson_multivariate(SBM & membership, const vec & vectorized)
// {
// }
...
/* If m_step(SBM_sym &, poisson_multivariate &, poisson_multivariate::network) is not defined
 * Then
 *     Usefull, Mandatory
 * Else
 *     Useless
...
// poisson_multivariate::poisson_multivariate(SBM_sym & membership, const vec & vectorized)
// {
// }
...
/* If m_step(LBM &, poisson_multivariate &, poisson_multivariate::network) is not defined
 * Then
 *     Usefull, Mandatory
 * Else
 *     Useless
...
// poisson_multivariate::poisson_multivariate(LBM & membership, const vec & vectorized)
// {
// }
...
// template<>
// inline
// vec grad(poisson_multivariate & model,
//          SBM & membership,
//          poisson_multivariate::network & net)
// {
// }
...
/* If one of the following specialization
 *     - grad(poisson_multivariate &, LBM &, poisson_multivariate::network)
 *     - grad(poisson_multivariate &, SBM &, poisson_multivariate::network)
 *     - grad(poisson_multivariate &, SBM_sym &, poisson_multivariate::network)
...
// template<>
// inline
// vec grad_logf(poisson_multivariate & model,
//               poisson_multivariate::network & net,
//               unsigned int i,
//               unsigned int j,
//               unsigned int q,
...
/* If grad_logf(poisson_multivariate &,
 *         poisson_multivariate::network &,
 *         unsigned int,
 *         unsigned int,
 *         unsigned int,
 *         unsigned int)
...
// inline
// double grad_logf(poisson_multivariate & model,
//                  poisson_multivariate::network & net,
//                  unsigned int i,
//                  unsigned int j,
//                  unsigned int q,
...
/* If one of the following specialization
 *     - PL(poisson_multivariate &, SBM &, poisson_multivariate::network)
 *     - PL(poisson_multivariate &, SBM_sym &, poisson_multivariate::network)
 *     - PL(poisson_multivariate &, LBM &, poisson_multivariate::network)
...
// inline
// double logf(poisson_multivariate & model,
//             poisson_multivariate::network & net,
//             unsigned int i,
//             unsigned int j,
//             unsigned int q,
